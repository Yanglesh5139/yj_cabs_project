...
VEHICLE_TYPE_MAPPING = [
    {'vehicle_type_id': 1, 'vehicle_type': 'YJX', 'description': 'Standard', 'base_rate': 2.50, 'per_mile': 1.75, 'per_minute': 0.35},
    {'vehicle_type_id': 2, 'vehicle_type': 'YJXL', 'description': 'Extra Large', 'base_rate': 3.50, 'per_mile': 2.25, 'per_minute': 0.45},
    {'vehicle_type_id': 3, 'vehicle_type': 'YJPOOL', 'description': 'Shared Ride', 'base_rate': 2.00, 'per_mile': 1.50, 'per_minute': 0.30},
    {'vehicle_type_id': 4, 'vehicle_type': 'YJ Comfort', 'description': 'Comfortable', 'base_rate': 3.00, 'per_mile': 2.00, 'per_minute': 0.40},
    {'vehicle_type_id': 5, 'vehicle_type': 'YJ Black', 'description': 'Premium', 'base_rate': 5.00, 'per_mile': 3.50, 'per_minute': 0.60}
]
...
# Extended locations for better UX
EXTENDED_LOCATIONS = [
    "Downtown Financial District", "Airport - JFK", "Silicon Valley Hub", "Midtown Manhattan",
    "Santa Monica Pier", "Chicago Loop", "Seattle Waterfront", "Boston Back Bay",
    "Austin Convention Center", "Denver Union Station", "Miami Brickell", "San Francisco SOMA",
    "Las Vegas Strip", "Orlando International", "Nashville Broadway", "Portland Pearl District"
] + CITY_LIST
...
def generate_YJ_ride_confirmation(origin: str = None, destination: str = None, vehicle_type: str = None, distance: float = None, duration: int = None):
    """
    Generates a complete ride confirmation matching your backend data structure.
    """
    # Generate timestamps
    pickup_time = datetime.now() - timedelta(days=random.randint(0, 30), hours=random.randint(0, 23))
    duration_minutes = duration if duration else random.randint(5, 120)
    ...
    # Distance in miles
    distance_miles = distance if distance else round(random.uniform(0.5, 50), 2)
    
    # Get vehicle details
    if vehicle_type and vehicle_type in VEHICLE_TYPE_ID_MAP:
        selected_vehicle = next((v for v in VEHICLE_TYPE_MAPPING if v['vehicle_type'] == vehicle_type), VEHICLE_TYPE_MAPPING[0])
    else:
        selected_vehicle = random.choice(VEHICLE_TYPE_MAPPING)
    
    vehicle_type_name = selected_vehicle['vehicle_type']
    vehicle_type_id = selected_vehicle['vehicle_type_id']
    
    # Pricing calculation based on actual vehicle rates
    ...
    distance_fare = round(distance_miles * per_mile_rate, 2)
    ...
    # Location details
    pickup_address = origin if origin else fake.address().replace('\n', ', ')
    dropoff_address = destination if destination else fake.address().replace('\n', ', ')
    ...
    # Determine cancellation status (10% chance)
    ...
    # Get ride status
    ride_status = random.choice(['Completed', 'Completed', 'Completed', 'Cancelled'])
    ...
    passenger_first = random.choice(first_names)
    passenger_last = random.choice(last_names)
    
    vehicle_models = ['Camry', 'Accord', 'Model 3', 'Civic', 'F-150', 'Malibu', 'X5', 'E-Class', 'A4', 'Escalade']
    vehicle_colors = ['Black', 'White', 'Gray', 'Silver', 'Blue', 'Red', 'Green', 'Burgundy']
    
    # Ride confirmation dictionary
    ride_confirmation = {
        # Keys/Identifiers
        ...
        'passenger_name': f"{passenger_first} {passenger_last}",
        'passenger_email': f"{passenger_first.lower()}.{passenger_last.lower()}{random.randint(1,999)}@example.com",
        'passenger_phone': fake.phone_number(),
        
        # Driver Information
        'driver_name': f"{driver_first} {driver_last}",
        ...
        'pickup_latitude': round(random.uniform(25, 48), 6),
        'pickup_longitude': round(random.uniform(-125, -65), 6),
        'dropoff_address': dropoff_address,
        'dropoff_latitude': round(random.uniform(25, 48), 6),
        'dropoff_longitude': round(random.uniform(-125, -65), 6),
...
def calculate_ride_estimate(origin: str, destination: str, vehicle_type: dict):
    """Calculate realistic fare estimate based on origin/destination"""
    # Generate deterministic distance based on location names
    hash_val = abs(hash(origin + destination)) % 100
    distance = round(2.5 + (hash_val % 25), 1)
    duration = max(8, min(75, round((distance / 18) * 60)))
    
    # Calculate fare
    base = vehicle_type['base_rate']
    distance_fare = distance * vehicle_type['per_mile']
    time_fare = duration * vehicle_type['per_minute']
    subtotal = base + distance_fare + time_fare
    surge = round(1.0 + (hash_val % 50) / 100, 2)
